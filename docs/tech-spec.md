...
Both arrangements share one fixed logical canvas size *per mode*, letterbox-scaled to fit the real viewport via a uniform `Math.min(w/logicalWidth, h/logicalHeight)` factor — this is what keeps card size proportionate to whichever axis binds, without ever hardcoding a pixel card size for a specific breakpoint. A 56-logical-unit toolbar band is reserved at the very top of both logical canvases (`TOOLBAR_HEIGHT`, folded into `rowY()`'s base offset) — wordmark/qualifier on the left, New Game/How to Play/Settings/About plus an EN/PT language toggle on the right, all drawn on the canvas itself (not DOM), so it scales and letterboxes as one unit with the table beneath it.

Landscape budgets its own, smaller edge margin and larger house-fan allowance than portrait (`LANDSCAPE_ROW_MARGIN = 24` vs portrait's `ROW_MARGIN = 60`; `LANDSCAPE_HOUSE_FAN_ALLOWANCE = 11 × HOUSE_OVERLAP_X` vs portrait's `6 ×`) — landscape's fit is height-bound, so shrinking its margin grows the card, and the width that frees up pays for a much roomier house fan, since the two are spent on different axes. Every house's fan peek is clamped (`houseFanPeek(cardCount, clear)` in `layout.ts`) rather than using a constant per-card peek: short houses still fan at the natural maximum, longer ones compress evenly against whichever allowance the active mode budgets, and a house long enough to hit a legibility floor stops shrinking further rather than ever overlapping a neighboring pile.

**Pile layout is a user setting, not purely a breakpoint** (Settings → Pile layout: SIDES/ROWS, default SIDES, persisted to `localStorage`). SIDES is the aspect-based choice described above. ROWS forces the portrait arrangement's six-row graph verbatim, regardless of the viewport's own aspect ratio — so a landscape-shaped window can still show the physical-table reading (cpu's three piles above, human's below) if the player prefers it, at the cost of a smaller card (fitting six rows in a height-bound viewport rather than four). This needs no separate geometry: it's implemented as which of the two existing `computeTableLayout` modes gets requested, not a third layout.
