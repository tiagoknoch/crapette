...
// without needing any game-playing intelligence (that's cpuPlayer.ts's job, later).
function playOneStep(state: GameState, rng: () => number): GameState {
...
function main(): void {
  const games = parseGamesArg(process.argv.slice(2));
  ...
  for (let i = 0; i < games; i++) {
    const seed = i + 1;
    try {
      const { state, moves } = playGame(seed);
